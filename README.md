...
![The top of the demo vault's brain-scan card: the counts, the summary, and the review pile broken down by cause](examples/demo-vault-run/card.png)

That is the card a live scan wrote for [`examples/demo-vault`](examples/demo-vault): a fictional, self-authored second brain of 250 notes, 251 files with the vault's own README, which the scan reads too (daily notes over three years, a greenhouse build, a sourdough log, a home network, a novel draft, reading notes, an archive with duplicates, an inbox and a personal folder). The full page is [`examples/demo-vault-run/report.html`](examples/demo-vault-run/report.html) and the rows it was rendered from are [`run.json`](examples/demo-vault-run/run.json) beside it; a card lists each note's vault-relative path, and this one names nothing but the demo.

**What the live run measured** (2026-09-24, `jev-1.13.0`, taxonomy `be00a24c7a68`): 229 of 251 notes sent, 6 decided locally, 16 withheld; 350,181 input tokens at 2.18 characters per token, **$0.0147**. The pre-flight had estimated $0.0084 to $0.0114 on the 3.8-to-2.8 range, so the bill came in 29% over the pessimistic end: short notes with dates, numbers and links tokenize more densely than prose, and this vault is mostly those. 27 notes (12%) went to the review pile. In 26 of them neither of the top two buckets was `needs_review`; the commonest pairs were `ephemeral` against `log_entry` (7 notes), `durable_memory` against `project_decision` (6) and `durable_memory` against `ephemeral` (5). In the 27th, `needs_review` was the runner-up, and no pile note voted `needs_review`. One note was quarantined for the fake database password planted in it; the checklist's first action is a taxonomy edit. Nothing in the vault is real, so the card is safe to look at.

Reproduce it offline, for free, in under a minute (the votes are the keyword fixture's, and the header says so):

```bash
jev-janitor examples/demo-vault --offline --html demo-card.html
```

Or live, with a key, for under two cents:

```bash
jev-janitor examples/demo-vault --plan            # the estimate first; nothing is sent
jev-janitor examples/demo-vault --json --html demo-card.html > demo-run.json
```
...
- the vault-relative path in posix form, redacted like the title one folder or file name at a time with the separators kept: never the absolute path, never your username or directory tree. A denylisted name matches across spaces, hyphens and underscores, so the name in each of `jane-doe.md`, `Jane_Doe.md` and a `[[jane-doe]]` link becomes `[NAME]` (`[NAME].md`, `[NAME].md` and `[[[NAME]]]`; through 0.4.10 only the spaced form did, so a name in a filename or a wikilink left as written while the title read `[NAME]`); since 0.5.5 the match also spans a dot, a slash and an en or em dash (`Jane.Doe`, `Jane/Doe`, `Jane–Doe`; through 0.5.4 those left as written); exactly these may stand between the words: whitespace, `_`, `-`, `.`, `/`, an en dash and an em dash. Forms it does not match include any other joiner (`Jane, Doe`, `Jane;Doe`, `Jane+Doe`, `Jane|Doe`), the surname-first form (`Doe, Jane`), a middle initial (`Jane Q. Doe`), the name run together (`JaneDoe`) and emphasis inside it (`**Jane** Doe`); add the forms your notes use as their own denylist entries. (Through 0.4.6 the path left as written, so a denylisted name in a filename went out in the clear while the title showed `[NAME]`, and a key in a folder name never quarantined. A ten-digit run in a filename now reads as `[PHONE]`, which over-redacts an identifier in the safe direction.) A filename stem that the redactor would change is used as the fallback title as written, not with its `-` and `_` turned into spaces, so a token-shaped filename with no title and no H1 is redacted rather than split into words no pattern matches; and a note whose own title holds a credential is never listed as a sibling title (through 0.4.6 such a title went out in the clear in every neighbour's list) The report's own `path` column is local and stays as written
- the names of frontmatter keys, redacted like the title, never their values (through 0.4.6 the names left as written), except the tool's own `janitor` key, which says nothing about the note: through 0.4.10 it was sent, so a note's first stamp added a key name and changed its cache key, and `--resume` after an `--apply` run re-sent every note stamped for the first time (a note already carrying the block kept its key); the other half of that bug, an undated note's age moving with the stamp's own write, lasted through 0.5.1 and is described under Large vaults
...
jev-janitor pre-flight: 24 of 39 notes under vault/ will be sent to TypeSafe
  ...
  bill (estimate): 24 notes to send, 0 served from the journal, 1 decided locally, 0 locked, 14 skipped, 0 undecodable, 0 unreadable
    ...
    age: 25 of 25 readable notes (100%) take their age from the filesystem (no creation date in their frontmatter, none recorded by a stamp); it resets if this vault is moved, cloned, restored or re-synced
...
- Writes are atomic: new content goes to a temp file next to the note's real file and is swapped in with `os.replace`, so a crash mid-write cannot leave a truncated note; a symlinked note is written through to its target and stays a link (through 0.4.10 the link itself was replaced by a regular file).
- A stamp resets the note's file-system creation time. The atomic write above makes the stamped note a new file, so the operating system's creation stamp (`st_birthtime`, what Obsidian's "created" sort and Dataview's `file.ctime` read) becomes the moment of the stamp: measured on Windows, three stamped notes moved by 4.5 s, to the run's time; macOS is expected to behave the same and is unmeasured. The janitor's own age reading is unaffected, because it uses a `created:` key or the date the stamp records (`created_from_mtime`), never the file-system creation time. If a tool of yours sorts by that stamp, give your notes a `created:` key before the first `--apply`; preserving the stamp is logged for a later release.
...
- Quarantine is a move, not a rewrite of the body: a note whose header parses is stamped with its `janitor:` block first, so the moved note says why it was moved, and everything else in it is byte-for-byte what it was (through 0.4.10 the docs said "unchanged", which was true of the body and not of the header); a note whose header does not parse is moved exactly as it is, unstamped, because a header the janitor cannot read is one it must not rewrite. In both cases the reason is in `manifest.jsonl`. The note goes to `_janitor/quarantine/<its vault-relative path>`, so two notes with the same file name in different folders stay distinct. The first move writes a `.gitignore` containing `*` into that folder, and every move appends a line to `_janitor/quarantine/manifest.jsonl` with the original path, the destination, the reason and the time.
...
- **A failed note does not end the run.** It becomes an `error` row with the exception name and message, the scan continues, and under `--apply` a note whose file cannot be written (held open by a sync client, or a header whose `janitor:` value is not a mapping) keeps its vote row with `applied: "error: <Type>"`, leaves no temp file behind, and the run goes on (through 0.4.10 one such note ended the run and every billed vote in flight lost its row), and the exit code is 2 so scripts can tell "finished with failures" from "declined" (exit 1). Once 50 notes have been sent, if more than 20% of all the notes sent so far have failed (a running total, checked after every send; through 0.4.7 the message said "of the first 50"), the run stops, because that is a broken key or an outage rather than bad notes.
...
The consent question is still asked whenever anything will be sent, and skipped only when nothing will. Edit the taxonomy, the denylist, a note's text within the cap, or a sibling's title, and the affected keys change, so those notes are re-sent; nothing is ever served for a changed input. If the API returns a different model version than the cached votes came from, the run stops: the pre-flight counted the cached notes as served, so they are not sent under the new model without a new estimate and a new consent (the first fresh vote reveals the change, so at most the notes already in flight are billed; the stop names both ways on: `--no-cache` re-judges every note under the new model, `--trust-cache-across-models` keeps serving the cached votes). Through 0.5.4 the run kept going and sent every cached note it had not served yet, which the pre-flight had never counted; a run never sends a note the pre-flight did not count as sent. The stop persists: a plain `--resume` after it refuses before the consent question, with the same two flags as the only ways on, because the stopped run's journal holds a vote from the new model and a resume that did not refuse would serve every old-model vote as if you had chosen to trust them. `--no-cache` retries errors but re-sends everything. `--journal-info` prints the newest journal and how many notes it would serve today, with no key.
...
**Findings: notes your tools cannot read.** A header between `---` fences that YAML cannot parse (`status: blocked: waiting on review` is the usual shape, and agents write it) is a fact about the vault, not a failure of the run. Such a note is judged on its body, with no frontmatter keys and none of the unreadable header text sent; its row carries `findings: ["unreadable_frontmatter"]` and the parser's reason; the pre-flight counts them and the run's closing lines total them. `--apply` leaves that header exactly as it is, because a header the janitor cannot read is one it must not rewrite (the residual: a note that opens with a rule of four or more dashes whose following lines happen to parse as a YAML mapping is read as frontmatter and stamped as such; a quoted `"janitor":` key is the same key, and the header keeps its own line ending whatever the body uses, both since 0.5.0), though a note holding a credential is still quarantined, since a move needs no parse. A file that is not valid UTF-8 is the other finding: a skip row with `findings: ["undecodable"]`, never sent, never retried. Neither is an `error` row; error rows are for failures of the run, which `--resume` retries. A third finding needs no new state: the share of judged notes whose age comes from the filesystem (no parseable creation date in their frontmatter and no date recorded by a janitor stamp), which resets if the vault is moved, cloned, restored or re-synced. Each row says `age_source: frontmatter`, `stamp` (the date the janitor recorded at the note's first stamp, which does not reset) or `mtime`; the pre-flight and the run's closing lines count the `mtime` ones against all three. (On one real 295-note sample it was 54%, and no tool the owner had ever mentioned it.)

**The brain-scan report.** `--html FILE` writes one self-contained page from the run's own rows (native font stacks, no stylesheet, script or font fetched: opening it makes no network request; through 0.5.1 it loaded three families from Google Fonts): a summary, the review pile broken down by cause with a fix for each, where the notes landed, the membrane, errors, cost against the measured ratio, graph and age, and a next-scan checklist ordered by impact, every number computed from the rows and every estimate labelled, with one quoted exception, the noise floor in the footer, which is a measurement from the calibration corpus, and a second one when it is used: the measured characters-per-token figure uses the question characters each call carried, recorded on the rows since 0.5.2; a run saved before that (the committed demo run is one) gets the default taxonomy's size only when its fingerprint is the default's, the footer then names that figure as the second not from the rows, and otherwise the page says the figure is unavailable (through 0.5.1 a constant fit only the default taxonomy, so a `--taxonomy` run's card showed a wrong ratio). It is a local file: it lists each note's vault-relative path as written, unredacted (the redacted form is what was sent), so a denylisted name or a token in a filename appears on the page; it never carries the excerpt, the frontmatter values or the aliases. Keep it outside the vault (the run warns if it is inside); it is never written to the journal, and it is never sent. An offline run's report says in its header that the votes are the keyword fixture's. A saved run renders again without a request: `python -m janitor.report run.json report.html "Vault name"`.
...
- **Large vaults.** Sequential is about a quarter of a second per note, so pass `--workers`. Progress is visible, every finished note is on disk as it completes, one failed note never discards the run, and `--resume` continues an interrupted run without re-sending what is done, an `--apply` run included: a stamp does not change a note's cache key, because the tool's own key name is not sent and an undated note's age is read from the date the stamp recorded (`janitor.created_from_mtime`, the file's mtime before the janitor's first write) rather than from the mtime the write moved; the file's mtime itself is left at the write, so sync clients see the change (through 0.5.1 an undated note's age band moved with the stamp and `--resume` after `--apply` re-sent it; a note stamped by 0.5.0 or 0.5.1 carries no recorded date, keeps the key it has, and gets the date on its next re-stamp) (under `--apply`, a note's file is stamped or moved by the one writer thread right before its row is recorded, never by a worker, so a Ctrl-C leaves at most one stamped note without a row, the one it landed on; through 0.4.7, since 0.4.6, workers wrote their own files ahead of the writer, so an interrupt inside the writer could leave several stamped or moved notes with no row, which `--resume` then re-sent). The prepare phase reads every note before the first request, which on a 20,000-note vault is some seconds of local I/O with no progress line yet.
