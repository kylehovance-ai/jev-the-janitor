...
- the vault-relative path in posix form: never the absolute path, never your username or directory tree
...
- the titles of the other notes in the same folder, for duplicate detection, with sensitive paths excluded unconditionally: the 80 closest by word overlap with the note's own title, each cut to 80 characters (through 0.3.0 it was the alphabetical first 80, uncut, so in a large folder a near-duplicate was never listed)
- graph facts as numbers, computed locally from one index pass: word, heading and link counts, in-degree, embeds, age in days, `is_moc`, `is_orphan`. Age is *creation* age: days since the earliest parseable `created` or `date` in the frontmatter, with the file's mtime used only when neither parses, so editing a note does not make it younger and a freshly cloned vault does not read as brand new. (Through 0.3.0 it was days since last touched, which made `is_orphan` unreachable on any cloned, restored or synced vault.) `is_orphan` is reported as unknown, not as a count, when fewer than one scanned note in twenty links out at all: on a vault that references notes by path rather than by link, in-degree is zero everywhere and says nothing, and the run's last lines say so. A subfolder scan also cannot see links from the rest of the vault. Counts only: the facts name no link target, and a link into a sensitive or hidden path is counted with its name omitted. (A `[[wikilink]]` you wrote in a body is body text and leaves inside the excerpt like any other text; that was always so.) Words are sent to two significant figures and age as a band (0, 1, 7, 30, 90, 365, 1000 days), so an edit below the excerpt cap keeps a note's cache key, and a key changes at midnight only on the day a note crosses a band edge: on any given day that is the notes turning exactly 1, 7, 30, 90 or 365 days old, about half a percent of a vault spread over three years. (Measured on a real 26-note vault: 25 of 26 keys stable across a day; the one that moved had turned a band age.)
- the frontmatter `aliases`, redacted exactly like the title. This is the one frontmatter value that is sent: an alias is a second title
...
```
jev-janitor pre-flight: 23 of 41 notes under vault/ will be sent to TypeSafe
  folders included (2):
    notes/                                     12 notes
...
- A note that is not valid UTF-8 is never sent, becomes an `error` row, and is left untouched; the run continues.
...
`taxonomy` is a short hash of the taxonomy file that produced the vote, so a stamp can be interpreted after the wording changes. The `--json` report additionally carries `bucket_probabilities`, `bucket_margin` (top-1 minus top-2), `exact_duplicate_of` (the first note with identical content, computed locally by hash), `title_sent` and `sibling_titles_sent` (what left, after redaction), `redacted_own` (the subset of `redacted` found in this note itself rather than in a neighbour's title -- only that subset can quarantine it) and `quarantine_triggers`. A note whose stamp did not need to change reports `applied: unchanged` and is not rewritten, so a re-scan does not dirty every note in git.
...
- **A failed note does not end the run.** It becomes an `error` row with the exception name and message, the scan continues, and the exit code is 2 so scripts can tell "finished with failures" from "declined" (exit 1). If more than 20% of the first 50 sent notes fail, the run stops, because that is a broken key or an outage rather than bad notes.
...
- It records **decisions, never what was sent**: no excerpt, no sibling titles. Each row carries the bucket, the probabilities, the action, the taxonomy fingerprint, and a `key` that hashes exactly the state that was sent, so a later run can tell whether a vote is still the vote it would get today.
...
Most of a large vault is arithmetic, and arithmetic is code's job. Before any request, one pass over the vault builds an index (every scanned note read exactly once; notes on sensitive or hidden paths are never opened), and four kinds of note are decided there with no model and no request:
...
| a high-precision secret hit (`KEY`, `JWT`, `PEM`, `AWS_SECRET`, `BEARER`) in the note's own title or body | `needs_review`, action `quarantine` | a credential format matched by regex is not a question for a model. `EMAIL`, `PHONE`, `CARD` and `NAME` are not in this list: they false-positive on prose, and the note goes to Jev with the tokens in place |
...
| change | fingerprint | effect on the 21 scans |
|---|---|---|
| baseline | `869d90f3047f` | split 9 project_decision / 7 durable_memory / 5 ephemeral, median confidence 0.35 |
