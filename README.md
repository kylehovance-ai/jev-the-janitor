...
**The brain-scan report.** `--html FILE` writes one self-contained page from the run's own rows: a summary, the review pile broken down by cause with a fix for each, where the notes landed, the membrane, errors, cost against the measured ratio, graph and age, and a next-scan checklist ordered by impact, every number computed from the rows and every estimate labelled. It is a local file: it lists each note's vault-relative path as written, unredacted (the redacted form is what was sent), so a denylisted name or a token in a filename appears on the page; it never carries the excerpt, the frontmatter values or the aliases. Keep it outside the vault (the run warns if it is inside); it is never written to the journal, and it is never sent. An offline run's report says in its header that the votes are the keyword fixture's. A saved run renders again without a request: `python -m janitor.report run.json report.html "Vault name"`.
