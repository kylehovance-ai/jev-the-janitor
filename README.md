...
- the titles of the other notes in the same folder, for duplicate detection, with sensitive paths excluded unconditionally: the 80 closest by word overlap with the note's own title, each redacted and then cut to 80 characters (through 0.3.0 it was the alphabetical first 80, uncut, so in a large folder a near-duplicate was never listed; through 0.4.6 the cut came before redaction, so a name or key straddling the cut left as a fragment with no token)
